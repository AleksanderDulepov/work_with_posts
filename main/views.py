...
def posts_list_page():
    ...
    if list_ != None:  # проверка возникновения ошибок при чтении файла json с постами
        ...
        return render_template("common_templates/error_page.html",
                               error_name=f'Постов по запросу "{word_to_search}" не найдено!',
    ...
    return render_template("common_templates/error_page.html", error_name="Ошибка при загрузке файла posts.json",
                           error_description='Вероятно, файл отсутствует или его содержимое невозможно прочитать',
                           page_back='/')
